...
import re, os, io, time, csv, statistics, argparse, itertools
import numpy as np
...
parser.add_argument('--trainfeats', type=list, help='features for training')
        ...
        for row in csvreader2:
            ...
            if cur_lem != row[0]:
                for feats in cur_forms:
                    ...
                    feats_list.pop(0)
                    feats_string = ' '.join(feats_list)
                    if feats == (' '.join(trainfeats)):
                        testset.write(f'<start> {charword.lower()} <end>\t<start> {lem_string} {feats_string} <end>\n')
                    elif ' '.join(trainfeats) not in cur_forms:
                        trainset.write(f'<start> {charword.lower()} <end>\t<start> {lem_string} {feats_string} <end>\n')
