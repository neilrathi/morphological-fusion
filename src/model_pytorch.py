...
# remove if not running in colab
...
units = 1024
learning_rate = 0.002
MAX_EPOCHS = 10 # if early stopping does not occur, stop training at MAX_EPOCHS

f = open(f'{filepath}/{lang}/{lang}.txt', encoding='UTF-8').read().strip().split('\n')
lines = f
...
class LanguageIndex():
    ...
    def create_index(self):
        ...
        for index, word in enumerate(self.vocab):
            self.word_id[word] = index + 1 # +1 because of pad token
        
        # id to word
        for word, index in self.word_id.items():
...
input_tensor = [[inp_lang.word_id[s] for s in lemma.split(' ')]  for lemma in traindata["lemma"].values.tolist()]
target_tensor = [[targ_lang.word_id[s] for s in form.split(' ')]  for form in traindata["form"].values.tolist()]
...
vocab_inp_size = len(inp_lang.word_id)
vocab_tar_size = len(targ_lang.word_id)
...
def sort_batch(X, y, lengths):
    lengths, index = lengths.sort(dim=0, descending=True)
    X = X[index]
    y = y[index]
...
for epoch in range(MAX_EPOCHS):
    ...
    for (batch, (inp, targ, inp_len)) in enumerate(dataset):
        ...
        # teacher forcing
        dec_input = torch.tensor([[targ_lang.word_id['<start>']]] * BATCH_SIZE)
...
def evaluate(features):
    ...
    inputs = [inp_lang.word_id[i] for i in features.split(' ')]
    len_list = [len(inputs)]
    ...
    inputs_batch = torch.unsqueeze(inputs, 1)

    result = ''

    print(inputs_batch.size())
    print(len_tensor.size())
    encoder_out, encoder_hidden = encoder(inputs_batch.to(device), len_tensor, device)

    dec_hidden = encoder_hidden
    dec_input = torch.unsqueeze([targ_lang.word_id['<start>']], 0)

    # iterate through maximum possible length of output
    for t in range(max_length_tar):
        predictions, dec_hidden, attention_weights = decoder(dec_input,
                                        dec_hidden, encoder_out)
        ...
        if targ_lang.id_word[predicted_id] == '<end>':
            return result
        # convert id to token, then add to result
        else:
            result += targ_lang.id_word[predicted_id]
...
def get_fusion(features, word):
    ...
    for t in range(len(wordlist)-1):
        ...
        word_id = targ_lang.word_id[wordlist[t]]
