...
def subsets(infile, trainfile, testfile, features, numfeats):
    with open(infile, 'r') as csvfile:
        ...
        for row in csvreader:
            ...
            if ((features[1] in rowfeats) and (features[2] in rowfeats)):
...
featsfile_io = os.path.join(filepath, 'featuresfile.txt')
...
with open(languagefile_io, 'r') as languagefile:
    ...
    for row in langreader:
        if not row: # empty rows
            continue
        row_feats = (row[2]).split(';')
        row_pos = row_feats[0]
        row_feats.pop(0)
        feat_tuples = list(itertools.combinations(row_feats, 2))
        for i in feat_tuples:
            if i not in feats_seen:
                featsfile.write(f"{row_pos};{';'.join(list(i))}\n")
                feats_seen.add(i)
    featsfile.close()
...
for trainfeats in featureslist:
    ...
    modelpath = os.path.join(filepath, modelfile)
    if not os.path.exists(modelpath):
        os.makedirs(modelpath)

    trainfile = os.path.join(filepath, modelfile, f'{modelfile}_train.txt')
    testfile_io = os.path.join(filepath, modelfile, f'{modelfile}_test.txt')
    resultsfile_io = os.path.join(filepath, modelfile, f'{modelfile}_results.txt')

    # directory for models
    checkpoint_dir = os.path.join(filepath, 'saved_models', f'{modelfile}_weights/')
    ...
    checkpoint_prefix = os.path.join(checkpoint_dir, f'{modelfile}_checkpoint')
    ...
    surpfile = os.path.join(filepath, f'{language}_surprisals.txt')
